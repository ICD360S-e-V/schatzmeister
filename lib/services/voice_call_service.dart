...
class VoiceCallService {
  ...
  static const Map<String, dynamic> _iceServers = {
    'iceServers': [
      ...
      // TURN server on icd360sev.icd360s.de (for relay when peer-to-peer fails)
      {
        ...
        'username': 'icd360s',
        'credential': 'n/RsfLEzkGJ97JAqtZDjyuwms4WdgXh8A40HrpTx7xU='
      },
      {
  ...
  Future<MediaStream?> _getLocalStream() async {
    try {
      ...
      // Select default audio device explicitly (Windows fix - GitHub issue #1628)
      final selectedDevice = audioInputs.first;
      _log.info('VoiceCallService: ✓ Using microphone: "${selectedDevice.label}" (${selectedDevice.deviceId})', tag: 'CALL');

      // Use explicit device selection with advanced constraints (Windows fix)
      final constraints = {
        'audio': {
          'deviceId': selectedDevice.deviceId, // EXPLICIT device selection for Windows
      ...
      _log.info('VoiceCallService: Audio tracks in stream: ${audioTracks.length}', tag: 'CALL');
      for (var i = 0; i < audioTracks.length; i++) {
        final track = audioTracks[i];
        _log.info('VoiceCallService: Audio track $i: enabled=${track.enabled}, muted=${track.muted}, kind=${track.kind}', tag: 'CALL');
      }
