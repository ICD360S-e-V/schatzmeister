...
class _TicketDetailsDialogState extends State<TicketDetailsDialog> with SingleTickerProviderStateMixin {
    ...
    if (entry != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.timerStarted(_selectedCategory.display)), backgroundColor: Colors.green),
        );
      }
    } else if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l.timerCouldNotStart), backgroundColor: Colors.red),
      );
    }
  }
  ...
  Future<void> _stopTimer() async {
    ...
    if (entry != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.timerStopped(entry.durationDisplay)), backgroundColor: Colors.green),
        );
      }
    }
  }
  ...
  Future<void> _deleteTimeEntry(TimeEntry entry) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(l.timeEntryDeleteConfirm(entry.category.display, entry.durationDisplay)),
        actions: [
        ...
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text(l.categoryLabelBold, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                ...
                Text(l.durationLabel, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: TextField(
                      ...
                      decoration: InputDecoration(labelText: l.hoursLabel, border: const OutlineInputBorder()),
                    )),
                    const SizedBox(width: 12),
                    Expanded(child: TextField(
                      controller: minutesCtrl,
                      keyboardType: TextInputType.number,
                      decoration: InputDecoration(labelText: l.minutesLabel, border: const OutlineInputBorder()),
                    )),
                  ],
                ),
                const SizedBox(height: 16),
                Text(l.date, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                const SizedBox(height: 8),
                OutlinedButton.icon(
                  onPressed: () async {
                ...
                TextField(
                  controller: noteCtrl,
                  decoration: InputDecoration(labelText: l.noteOptional, border: const OutlineInputBorder()),
                  maxLines: 2,
                ),
              ],
            ),
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (totalMinutes < 1) {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(l.minOneMinute), backgroundColor: Colors.red),
                    );
                  }
                  return;
                }
                ...
                if (entry != null) {
                  ...
                  if (mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(l.timeEntrySaved(durationStr)), backgroundColor: Colors.green),
                    );
                  }
                } else {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(l.errorSavingTimeEntry), backgroundColor: Colors.red),
                    );
                  }
                }
              },
  ...
  Future<void> _submitComment() async {
    ...
    if (mounted) {
      ...
      if (comment != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.errorAddingComment),
  ...
  Future<void> _pickAndUploadFile() async {
    ...
    if (result.files.length > 20) {
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.max20FilesAllowed),
            backgroundColor: Colors.red,
          ),
        );
      }
    ...
    if (totalSize > 100 * 1024 * 1024) {
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.totalSizeExceeds(sizeMB)),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _uploadMultipleFiles(List<PlatformFile> files) async {
          ...
          return AlertDialog(
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text(
                  l.filesUploading(uploaded + failed, total),
                ...
                if (uploaded > 0 || failed > 0) ...[
                  const SizedBox(height: 8),
                  Text(
                    l.uploadResult(uploaded, failed),
    ...
    if (mounted) {
      ...
      if (failed == 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.filesUploadedSuccess(uploaded)),
            backgroundColor: Colors.green,
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.uploadedFailed(uploaded, failed)),
  ...
  Future<void> _captureWithImagePicker() async {
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.cameraNotAvailable(e.message ?? '')),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _uploadFile(String filePath) async {
    ...
    if (mounted) {
      ...
      if (attachment != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.fileUploaded(attachment.originalFilename)),
            backgroundColor: Colors.green,
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.errorUploadingFile),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  String _formatDate(DateTime date) {
    ...
    if (diff.inMinutes < 1) return l.justNowShort;
    if (diff.inMinutes < 60) return l.minAgo(diff.inMinutes);
                    ...
                    GestureDetector(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Tooltip(
                            message: _showOriginalSubject ? AppLocalizations.of(context).showTranslation : AppLocalizations.of(context).showOriginal,
              ...
              builder: (context) {
                ...
                return TabBar(
                  ...
                  tabs: [
                    ...
                    Tab(icon: const Icon(Icons.folder_outlined), text: l.documentsTabTicket),
                    Tab(icon: const Icon(Icons.timer_outlined), text: l.timeTrackingTab),
                    Tab(icon: const Icon(Icons.history), text: l.historyTab),
                  ],
                );
              },
            ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.ticket.memberName ?? AppLocalizations.of(context).unknown,
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          _showOriginalMessage ? AppLocalizations.of(context).translationLabel : AppLocalizations.of(context).originalLabel,
          ...
          // Dates
          _buildInfoRow(Icons.calendar_today, AppLocalizations.of(context).createdDateLabel, _formatDate(widget.ticket.createdAt)),
          if (widget.ticket.updatedAt != null)
            _buildInfoRow(Icons.update, AppLocalizations.of(context).updatedDateLabel, _formatDate(widget.ticket.updatedAt!)),
          if (widget.ticket.adminName != null)
            _buildInfoRow(Icons.support_agent, AppLocalizations.of(context).processorLabel2, widget.ticket.adminName!),
          if (widget.ticket.closedAt != null)
            _buildInfoRow(Icons.check_circle, AppLocalizations.of(context).completedDateLabel, _formatDate(widget.ticket.closedAt!)),
                        ...
                        onPressed: () async {
                          ...
                          if (picked != null) {
                            ...
                            if (result != null && mounted) {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(AppLocalizations.of(context).ticketScheduledFor(DateFormat('dd.MM.yyyy HH:mm').format(dt))),
                                  backgroundColor: Colors.green,
                                ),
                              );
                            }
                        ...
                        onPressed: () async {
                          ...
                          if (picked != null) {
                            ...
                            if (result != null && mounted) {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(AppLocalizations.of(context).timeChangedTo(picked.format(context))),
                                  backgroundColor: Colors.green,
                                ),
                              );
                            }
                ...
                Wrap(
                  ...
                  children: [
                    if (widget.ticket.status == 'open')
                      _buildStatusButton(
                        label: AppLocalizations.of(context).assignTicket,
                    ...
                    if (widget.ticket.status != 'waiting_member')
                      _buildStatusButton(
                        label: AppLocalizations.of(context).waitingForUser,
                    ...
                    if (widget.ticket.status != 'waiting_staff')
                      _buildStatusButton(
                        label: AppLocalizations.of(context).waitingForStaff,
                    ...
                    if (widget.ticket.status != 'waiting_authority')
                      _buildStatusButton(
                        label: AppLocalizations.of(context).waitingForAuthority2,
                    ...
                    if (widget.ticket.status != 'waiting_documents')
                      _buildStatusButton(
                        label: AppLocalizations.of(context).waitingForDocuments,
                    ...
                    if (widget.ticket.status != 'open' && widget.ticket.status == 'done')
                      _buildStatusButton(
                        label: AppLocalizations.of(context).reopenTicket,
                        icon: Icons.replay,
                        color: Colors.orange,
                        action: 'reopen',
                      ),
          ...
          child: Column(
            ...
            children: [
              ...
              CheckboxListTile(
                ...
                title: Text(AppLocalizations.of(context).internalComment, style: const TextStyle(fontSize: 13)),
                subtitle: Text(
                  AppLocalizations.of(context).onlyVisibleForAdmins,
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      Text(
                        AppLocalizations.of(context).noDocumentsTicket,
  ...
  Future<void> _openAttachment(TicketAttachment attachment) async {
    ...
    if (mounted) {
      ...
      if (filePath != null) {
        ...
        if (!handledInApp) {
          try {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(l.errorOpeningFile),
                  backgroundColor: Colors.red,
                ),
              );
            }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.errorDownloadingFile),
            backgroundColor: Colors.red,
          ),
        );
      }
        ...
        children: [
          ...
          GestureDetector(
            onDoubleTap: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(AppLocalizations.of(context).commentCopied),
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text(
                    AppLocalizations.of(context).internBadge,
                    style: TextStyle(
                      fontSize: 10,
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      if (file != null && file.bytes != null) {
        ...
        setState(() {
          _error = AppLocalizations.of(context).photoCouldNotBeTaken;
          _isCapturing = false;
        });
      }
    } catch (e) {
      setState(() {
        _error = AppLocalizations.of(context).errorMessage('$e');
        _isCapturing = false;
      });
    }
  }
  ...
  void _onCropped(CropResult result) {
    switch (result) {
      ...
      case CropFailure(:final cause):
        setState(() {
          _error = AppLocalizations.of(context).cropFailed('$cause');
          _isCropping = false;
        });
    }
  }
          ...
          Expanded(
            child: Text(
              _step == _CameraStep.camera
                  ...
                  : _step == _CameraStep.crop
                      ? AppLocalizations.of(context).cropPhoto
    ...
    switch (_step) {
      case _CameraStep.camera:
        return [
          ...
          ElevatedButton.icon(
            ...
            label: Text(AppLocalizations.of(context).captureLabel),
      ...
      case _CameraStep.crop:
        return [
          TextButton(
            onPressed: _retake,
            child: Text(AppLocalizations.of(context).retakeLabel),
          ),
          TextButton(
            onPressed: _skipCrop,
            child: Text(AppLocalizations.of(context).skipLabelBtn),
          ),
          ElevatedButton.icon(
            icon: _isCropping
                ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                : const Icon(Icons.crop, size: 16),
            label: Text(AppLocalizations.of(context).cropLabel),
      ...
      case _CameraStep.preview:
        return [
          TextButton(
            onPressed: () => Navigator.pop(context, null),
            child: Text(AppLocalizations.of(context).discardLabel),
          ),
          TextButton(
            onPressed: _backToCrop,
            child: Text(AppLocalizations.of(context).cropAgain),
          ),
          TextButton(
            onPressed: _retake,
            child: Text(AppLocalizations.of(context).retakeLabel),
  ...
  Widget _buildCropView() {
    ...
    return Column(
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          AppLocalizations.of(context).selectCropArea,
  ...
  Widget _buildPreview() {
    ...
    return Column(
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          AppLocalizations.of(context).uploadThisPhoto,
          style: TextStyle(color: Colors.grey.shade600, fontSize: 13),
        ),
      ],
    );
