...
class _AdminChatDialogState extends State<AdminChatDialog> {
      ...
      if (_selectedConversation != null &&
        ...
        _safeSetState(() {
          _messages.add({
            ...
            'is_own': message.senderId == _chatService.currentUserId || message.senderName == widget.userName,
  ...
  Future<void> _startCall() async {
    ...
    final memberName = _selectedConversation!['member_name'] ?? AppLocalizations.of(context).userLabel;
    ...
    try {
      ...
      if (mounted) {
        ...
        if (e.toString().contains('NO_MICROPHONE')) {
          ...
          _showError(l.errorStartingCall(e.toString()));
        }
      }
  ...
  Future<void> _handleCallAnswer(String sdp, String sdpType, String answererName) async {
    ...
    try {
      ...
      if (mounted) {
        final l = AppLocalizations.of(context);
        _showError(l.errorConnecting(e.toString()));
      }
      _endCallCleanup();
    }
  }
    ...
    try {
      ...
      if (mounted) {
        ...
        if (e.toString().contains('NO_MICROPHONE')) {
          ...
          _showError(l.errorAccepting(e.toString()));
        }
      }
      await _voiceCallService.endCall();
    }
  ...
  Future<void> _uploadFiles() async {
    ...
    try {
      ...
      if (result['success'] == true && mounted) {
        ...
        if (mounted) {
          final l = AppLocalizations.of(context);
          _showError(result['message'] ?? l.uploadFailed);
        }
      }
    } catch (e) {
      if (mounted) {
        final l = AppLocalizations.of(context);
        _showError('${l.uploadError}: $e');
      }
    } finally {
      if (mounted) {
        _safeSetState(() {
    ...
    try {
      ...
      if (result['success'] == true) {
        if (result['content'] != null) {
          ...
          final filePath = '${tempDir.path}/${result['filename']}';
          final file = File(filePath);
          await file.writeAsBytes(bytes);
          await OpenFilex.open(filePath);
        } else if (result['download_url'] != null) {
          // Large file - open download URL
          if (mounted) {
            final l = AppLocalizations.of(context);
            _showError(l.largeFileDownload);
          }
        }
      } else {
        if (mounted) {
          final l = AppLocalizations.of(context);
          _showError(result['message'] ?? l.downloadFailed2);
        }
      }
    } catch (e) {
      if (mounted) {
        final l = AppLocalizations.of(context);
        _showError('${l.downloadError}: $e');
      }
    }
  }
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      if (mounted) {
        final l = AppLocalizations.of(context);
        _showError(l.errorWith(e.toString()));
      }
    } finally {
  ...
  Future<void> _closeConversation() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(l.closeConversation),
        content: Text(l.closeConversationConfirm),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (result['success'] == true && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l3.conversationClosed),
      ...
      if (mounted) {
        final l3 = AppLocalizations.of(context);
        _showError(l3.errorWith(e.toString()));
      }
    }
  }
  ...
  void _showMuteOptions() {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(l.muteTitle),
        content: Text(l.muteDurationQuestion),
        actions: [
          TextButton(
            onPressed: () {
    ...
    try {
      ...
      if (result['success'] == true && mounted) {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(isMuted ? l.conversationMuted : l.muteRemoved),
              backgroundColor: Colors.green,
            ),
          );
        }
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: members.isEmpty
              ? Center(child: Text(l.noMembersFoundChat))
              : ListView.builder(
                  itemCount: members.length,
                  itemBuilder: (context, index) {
                    final member = members[index];
                    final memberNr = member['mitgliedernummer']?.toString() ?? '';
                    final name = member['name']?.toString() ?? l.unknown;
    ...
    try {
      ...
      if (result['success'] == true && mounted) {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(l.conversationStartedWith(memberName)),
              backgroundColor: Colors.green,
            ),
          );
        }
        ...
        final l = AppLocalizations.of(context);
        _showError(result['message'] ?? l.errorStartingConversation);
      }
    } catch (e) {
      if (mounted) {
        final l = AppLocalizations.of(context);
                      ...
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(l.noAutomaticMessages, style: TextStyle(color: Colors.grey.shade500)),
                              const SizedBox(height: 8),
                              Text(l.createReminders, style: TextStyle(color: Colors.grey.shade400, fontSize: 12)),
                            ],
                          ),
                        )
                      : ListView.separated(
  ...
  Future<void> _showAddScheduledMessageDialog() async {
    ...
    final categories = [
      {'value': 'fruehstueck', 'label': l.categoryBreakfast, 'icon': Icons.free_breakfast},
      {'value': 'mittagessen', 'label': l.categoryLunch, 'icon': Icons.lunch_dining},
      {'value': 'abendessen', 'label': l.categoryDinner, 'icon': Icons.dinner_dining},
      {'value': 'medikament', 'label': l.categoryMedicine, 'icon': Icons.medication},
      {'value': 'mahlzeit', 'label': l.categoryOther, 'icon': Icons.restaurant},
    ];
        ...
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                // Category
                Text(l.category, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade700)),
                ...
                Row(
                  children: [
                    Text('${l.time}: ', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade700)),
                    OutlinedButton.icon(
                ...
                const SizedBox(height: 12),
                // Days of week
                Text(l.weekdays, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade700)),
                const SizedBox(height: 6),
                Wrap(
                  spacing: 4,
                  children: [
  ...
  Future<void> _showConversationScheduledDialog(Map<String, dynamic> conversation) async {
    ...
    final memberName = conversation['member_name'] ?? l.unknown;
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(l.automaticMessages, style: const TextStyle(fontSize: 15)),
                      ...
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 12),
                              Text(l.noMessagesAvailable, style: TextStyle(color: Colors.grey.shade500)),
                              const SizedBox(height: 8),
                              Text(l.createMessagesFirst, style: TextStyle(color: Colors.grey.shade400, fontSize: 12)),
                            ],
                          ),
                        )
                      : ListView.builder(
