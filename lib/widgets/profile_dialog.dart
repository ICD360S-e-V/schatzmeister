...
class _ProfileDialogState extends State<ProfileDialog> with SingleTickerProviderStateMixin {
  ...
  Future<void> _downloadDokument(MemberDokument doc) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppLocalizations.of(context).savedFile(data['filename'])),
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context).errorSavingWith('$e')), backgroundColor: Colors.red),
      );
    }
  }
  ...
  Future<void> _saveStufe1() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).personalDataSaved), backgroundColor: Colors.green),
        );
        _loadProfileData();
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isSavingStufe1 = false);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).errorMessage('$e')), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Future<void> _saveStufe3({String? zahlungsmethode, int? zahlungstag}) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).paymentDataSaved), backgroundColor: Colors.green),
        );
        _loadProfileData();
      }
    } catch (e) {
  ...
  Future<void> _savePhone() async {
    ...
    if (_phoneNumberController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l.pleaseEnterPhoneNumber),
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.phoneNumberSaved),
            backgroundColor: Colors.green,
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? l.errorSaving),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _revokeSession(int sessionId, String deviceName) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(l.logoutDevice),
        content: Text(l.logoutDeviceConfirm(deviceName)),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (result['success'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.deviceLoggedOutSuccess),
            backgroundColor: Colors.green,
          ),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? l.errorLoggingOut),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  String _getRoleText(String role, AppLocalizations l) {
    switch (role) {
      case 'vorsitzer':
        return l.roleVorsitzer;
      case 'schatzmeister':
        return l.roleSchatzmeister;
      case 'kassierer':
        return l.roleKassierer;
      case 'mitgliedergrunder':
        return l.roleGruender;
      default:
        return role;
    }
  }
  ...
  Future<void> _changePassword() async {
    ...
    if (_newPasswordController.text != _confirmPasswordController.text) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l.passwordsDoNotMatchError),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    if (_newPasswordController.text.length < 8) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l.passwordMin8Chars),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    try {
      ...
      if (result['success'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.passwordChangedSuccess),
            backgroundColor: Colors.green,
          ),
        );
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? l.errorChangingPassword),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _changeEmail() async {
    ...
    if (_newEmailController.text.isEmpty || !_newEmailController.text.contains('@')) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l.pleaseEnterValidEmailAddress),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.emailChangedSuccess),
            backgroundColor: Colors.green,
          ),
        );
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? l.errorChangingEmail),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget _buildProfileTab() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          // Profile Info
          _buildInfoRow(Icons.badge, l.memberNumber, widget.mitgliedernummer),
          const SizedBox(height: 12),
          _buildInfoRow(Icons.email, l.email, widget.email),
          const SizedBox(height: 12),
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text(
              session['device_name'] ?? l.unknownDeviceName,
            ...
            subtitle: Column(
              ...
              children: [
                ...
                Text(l.lastSeenLabel(_formatLastSeen(session['last_used']))),
                if (isCurrentSession)
                  Padding(
  ...
  Widget _buildVerwarnungenTab() {
    final l = AppLocalizations.of(context);
    if (widget.userId == null) {
      return _buildNoDataTab(Icons.warning_amber, l.warningsNotAvailable);
    }
  ...
  Widget _buildDokumenteTab() {
    final l = AppLocalizations.of(context);
    if (widget.userId == null) {
      return _buildNoDataTab(Icons.folder_open, l.documentsNotAvailable);
    }
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              l.uploadedBy(doc.uploadedByName),
                              style: TextStyle(fontSize: 11, color: Colors.grey.shade500),
                            ),
                          ],
                        ),
  ...
  Widget _buildMitgliedschaftTab() {
    ...
    final mitgliedsartLabels = {
      'ordentlich': l.ordinaryMember,
      'foerdermitglied': l.supportingMember,
      'ehrenmitglied': l.honoraryMember,
    };

    final zahlungsLabels = {
      'ueberweisung': l.bankTransfer,
      'sepa_lastschrift': l.sepaDirectDebit,
      'dauerauftrag': l.permanentOrder,
    };
      ...
      child: Column(
        ...
        children: [
          ...
          _mitgliedschaftRow(
            ...
            child: Text(
              createdAt != null ? dateFormat.format(createdAt) : l.unknown,
  ...
  Widget _buildVerifizierungTab() {
    final l = AppLocalizations.of(context);
    if (widget.userId == null) {
      return _buildNoDataTab(Icons.verified_user, l.verificationNotAvailable);
    }
              ...
              children: [
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text(
                      l.stagesVerified(geprueftCount, totalCount),
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        color: geprueftCount == totalCount ? Colors.green.shade700 : Colors.blue.shade700,
                      ),
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: stage['geprueft_am'] != null
            ? Text(
                l.checkedOnBy(_formatDate(stage['geprueft_am']), stage['geprueft_von_name'] ?? l.unknown),
  ...
  Widget _buildStufe1ReadOnlyContent() {
    ...
    final geburtsdatumStr = _selectedGeburtsdatum != null
        ...
        : '';
    final fields = [
      [l.firstName, _vornameController.text],
      [l.lastName, _nachnameController.text],
      [l.birthDate, geburtsdatumStr],
      [l.street, _strasseController.text],
      [l.houseNumber, _hausnummerController.text],
      [l.postalCode, _plzController.text],
      [l.city, _ortController.text],
      [l.phoneNumber, _telefonMobilController.text],
    ];
    ...
    return Column(
      ...
      children: [
        ...
        if (_selectedZahlungsmethode != null && _selectedZahlungsmethode!.isNotEmpty)
          ...
          Text(l.noPaymentMethodChosen, style: TextStyle(color: Colors.red.shade400, fontStyle: FontStyle.italic, fontSize: 13)),
          const SizedBox(height: 8),
          DropdownButtonFormField<String>(
            decoration: InputDecoration(
              labelText: l.selectPaymentMethod,
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
              contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
            ),
  ...
  String _formatLastSeen(dynamic lastSeen) {
    final l = AppLocalizations.of(context);
    if (lastSeen == null) return l.never;
    try {
      final date = DateTime.parse(lastSeen.toString());
      final now = DateTime.now();
      final difference = now.difference(date);

      if (difference.inSeconds < 60) return l.justNow;
  ...
  String _formatDate(String? dateStr) {
    final l = AppLocalizations.of(context);
    if (dateStr == null) return l.unknown;
  ...
  String _stufeName(int stufe) {
    return AppLocalizations.of(context).stageName(stufe);
  }
  ...
  String _verifizierungStatusText(String status) {
    final l = AppLocalizations.of(context);
    switch (status) {
      case 'geprueft': return l.statusVerified;
