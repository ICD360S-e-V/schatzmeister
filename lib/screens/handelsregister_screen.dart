  ...
  Future<void> _search() async {
    ...
    if (nummer.isEmpty && schlagwoerter.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l.enterRegisterOrKeywords),
      ...
      if (result['success'] == true) {
        ...
        if (regData != null) {
          ...
          setState(() {
            ...
            _isSearching = false;
            if (entries.isEmpty) {
              _error = regData['message'] ?? l.noResultsFound;
            }
          });
        } else {
          setState(() {
        ...
        setState(() {
          _isSearching = false;
          _hasSearched = true;
          _error = result['message'] ?? l.queryFailed;
        });
      }
    } catch (e) {
      if (mounted) {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(result['message'] ?? l.downloadFailed2),
      ...
      if (mounted) {
        final l = AppLocalizations.of(context);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l.errorWith(e.toString())), backgroundColor: Colors.red),
        );
      }
    } finally {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(l.queryingRegister, style: const TextStyle(fontSize: 13)),
          ],
        ),
      );
    }
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildDetail(Icons.account_balance, l.gerichtLabel, entry['register_gericht']),
                _buildDetail(Icons.tag, l.registerNr, entry['register_nummer']),
                _buildDetail(Icons.map, l.bundeslandLabel, entry['bundesland']),
                _buildDetail(Icons.business, l.sitzLabel, entry['sitz']),
                _buildDetail(Icons.info_outline, l.status, entry['status']),
                const SizedBox(height: 8),
                const Divider(height: 1),
                const SizedBox(height: 8),
                Text(l.documentsLabel, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey.shade600)),
                const SizedBox(height: 6),
                Wrap(
                  spacing: 6,
                  runSpacing: 6,
                  children: [
                    _buildDocChip(entry, 'AD', l.currentPrint),
                    _buildDocChip(entry, 'CD', l.chronologicalPrint),
                    _buildDocChip(entry, 'SI', l.structuredContent),
                    _buildDocChip(entry, 'DK', l.documentBasket),
                    _buildDocChip(entry, 'UT', l.companyHolder),
                    _buildDocChip(entry, 'VÖ', l.publications),
                  ],
                ),
              ],
            ),
